...
function M:RegisterForUnit(unitToken, reason)
	if not unitToken or not UnitExists(unitToken) or not units:IsEnemy(unitToken) then
		return 0
	end
...
function M:Init()
	...
	registerFrame:SetScript("OnEvent", function(_, event, arg1)
		...
		elseif event == "NAME_PLATE_UNIT_ADDED" then
			local zone = moduleUtil:GetZoneConfig()
			if zone and zone.TargetFocusOnly == false and arg1 and units:IsEnemy(arg1) then
